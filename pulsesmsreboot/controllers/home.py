...
from pulsesmsreboot.controllers.user_container import UserContainer
from pulsesmsreboot.controllers.settings import Settings
from pulsesmsreboot.controllers.drawer import Drawer
from pulsesmsreboot.model.user import UserDAO
from pulsesmsreboot.view.home import Ui_Home
...
class Home(QWidget, Ui_Home):
    ...
    def __init__(self):
        ...
        self.pulseSettings = Settings()
        # Account
        self.pulseSettings.emitDisableUser.connect(self.disableUserPage)
        self.pulseSettings.emitDeleteUser.connect(self.delUserPage)
        self.pulseSettings.emitEditUser.connect(self.editUserPage)
        self.pulseSettings.emitNewtUser.connect(self.addNewUser)
        # Personalization (Atribuição inversa, pois todos os componentes já existem)
        self.emitUpdateTheme = self.pulseSettings.emitUpdateTheme
        self.emitDisableTrayIcon = self.pulseSettings.emitDisableTrayIcon
        self.emitNotifications = self.pulseSettings.emitNotifications
        # Avanced
        self.pulseSettings.emitHideSettingsBar.connect(self.activeSettingsBar)
        self.emiHideNotificationCounter = self.pulseSettings.emiHideNotificationCounter
        # Quit
        self.emitQuit = self.pulseSettings.emitQuit
        self.pulseSettings.emitCloseSettings.connect(self.openSettings)
        # Open Pulse SMS Settings
        self.pulseSettings.emitOpenSettingsPulseSMS.connect(
            self.openPulseSMSSettings)
        # Update Pages
        self.pulseSettings.emitUpdateProxyPage.connect(
            self.reloadAllPages)
        ...
        self.drawer.raise_()
        self.drawer.stackedWidget.insertWidget(0, self.pulseSettings)
    ...
    def updateShortcuts(self):
        ...
        self.pulseSettings.accountPage.updateUsersShortcuts()
    ...
    def loadActionsMenuBar(self):
        ...
        def newAccount():
            from pulsesmsreboot.model.user import UserDAO
            from pulsesmsreboot.controllers.card_user import User
            from pulsesmsreboot.theme.builder_icon import getNewIconSVG
            LIMITE_USERS = 9
            ...
            if self.menu.count() < LIMITE_USERS:
                ...
                self.pulseSettings.accountPage.updateListUser(user)
    ...
    def openDonations(self):
        self.openSettings()
        self.pulseSettings.openDonations()
    ...
    def openPulseSMSSettings(self):
        i = self.userStacked.currentIndex()
        btn = self.menu.itemAt(i).widget()
        btn.openPulseSMSSettings()
        self.openSettings()
    ...
    def delUserPage(self, user):
        ...
        try:
            ...
            # Delete User Data
            path = os.path.join(pulsesmsreboot.path_storage, str(user.id))
