...
from pulsesmsreboot import __pulsesms_url__, __appname__
from pulsesmsreboot.services.dbus_theme import getSystemTheme


class PulseSMS(QWebEnginePage):
    ...
    # Solução alternativa ao acceptNavigationRequest, pois não funcionou dentro do pulse sms.
    # Mapeia os eventos do Mouse e abre o link a partir do capturado do signal linkHovered.
    def eventFilter(self, obj, event):
        if event.type() == QEvent.Type.MouseButtonPress:
            if event.button() == Qt.MouseButton.LeftButton:
                if self.link_url != '' and self.link_url != __pulsesms_url__ and not 'https://support.pulsesms.app/hc/en-us' in self.link_url:
