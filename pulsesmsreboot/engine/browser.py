...
from pulsesmsreboot import __appname__
from ..controllers.download_popup import DownloadPopup
from pulsesmsreboot.engine.pulsesms import PulseSMS
...
class Browser(QWebEngineView):
    ...
    def __init__(self, storageName, parent):
        super().__init__()
        self.qset = QSettings(pulsesmsreboot.__appname__, pulsesmsreboot.__appname__)
        ...
        if self.storageName == 1:
            self.storageName = 'storage-pulse'
        ...
        self.profile.setHttpUserAgent(pulsesmsreboot.__user_agent__)
        ...
        self.pulse = PulseSMS(self.profile, self)

        # Set PulseSMS page
        self.setPulseSMSPage()
    ...
    def setPulseSMSPage(self):
        self.stop()
        self.setPage(self.pulse)
        self.load(QUrl(pulsesmsreboot.__pulsesms_url__))
            ...
            if name in self.list_ignore:
                ...
                if name == 'Copy link address':
                    def setClipboard():
                        ...
                        cb.setText(self.pulse.link_context,
            ...
            if r == 1:
                ...
                if not self.qset.value("system/folderDownloads", False, bool):
                    directory = os.path.join(QStandardPaths.writableLocation(
                        QStandardPaths.StandardLocation.DownloadLocation), 'PulseSMS Downloads')
    ...
    def getPathImage(self, qin, title) -> str:
        ...
        """
        try:
            path = pulsesmsreboot.path_tmp+'/'+title+'.png'
